<div class="row">
	...
	$sql = "SELECT * FROM `nw_items` WHERE
		`iType` = '".$__ITEM['armor_head']['dbname']."' OR
		`iType` = '".$__ITEM['armor_body']['dbname']."' OR
		`iType` = '".$__ITEM['armor_hands']['dbname']."' OR
		`iType` = '".$__ITEM['armor_legs']['dbname']."'
		ORDER BY `iLevel` ASC";
	...
	$weapon_list = array();
	$sql = "SELECT * FROM `nw_items` WHERE
		`iType` = '".$__ITEM['weapon_gun']['dbname']."' OR
		`iType` = '".$__ITEM['weapon_white']['dbname']."' OR
		`iType` = '".$__ITEM['weapon_ammo']['dbname']."' OR
		`iType` = '".$__ITEM['weapon_thrown']['dbname']."'
		ORDER BY `iLevel` ASC";
	$query = mysql_query($sql);
	if(mysql_num_rows($query) > 0) {
		while($row = mysql_fetch_array($query)) {
	...
	if(isset($_POST['buyItem'])) {
		...
		if($_item > 0) {
			...
			header("Location: index.php?app=user&module=shop&section=equipment");
		}
	}
	...
	<form method="post" action="">
		<div class="col-md-6">
			<h3 class="text-center">Pancerz</h3>
